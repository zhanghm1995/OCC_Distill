...
@MIDDLE_ENCODERS.register_module()
class SparseEncoderLidarOCC(SparseEncoder):
    r"""Sparse encoder for Lidar occupancy prediction
...
@MIDDLE_ENCODERS.register_module()
class SparseEncoderWoDownsample(SparseEncoderLidarOCC):
    r"""The sparse encoder without feature downsampling.
                ...
                if i != 0 and j == 0 and block_type == 'conv_module':
                    blocks_list.append(
                        make_block(
                            ...
                            3,
                            norm_cfg=norm_cfg,
                            stride=1,  # we change this from 2 to 1
                            padding=padding,
                            indice_key=f'spconv{i + 1}',
                            conv_type='SparseConv3d'))
                elif block_type == 'basicblock':
