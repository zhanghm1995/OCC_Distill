...
Date: 2023-07-27 10:24:15
Email: haimingzhang@link.cuhk.edu.cn
Description: Use the separate convolution for nerf voxels.
'''
...
class MyBEVStereo4DOCCNeRFV1(BEVStereo4D):
    ...
    def __init__(self,
                 ...
                 refine_conv=False,
                 **kwargs):
        super(MyBEVStereo4DOCCNeRFV1, self).__init__(**kwargs)
        ...
        input_nerf_decoder = 1
        if self.NeRFDecoder.img_recon_head:
            input_nerf_decoder += 3
        if self.NeRFDecoder.semantic_head:
            input_nerf_decoder += self.NeRFDecoder.semantic_dim
        ...
        self.density_conv = nn.Sequential(
            *[
                ...
                nn.Conv3d(16, input_nerf_decoder, 1)
            ])
    ...
    def forward_train(self,
        ...
        if False:  # DEBUG ONLY!
            ...
            # volume feature
            output = self.density_conv(img_feats[0]).permute(0, 1, 4, 3, 2) # B, C, X, Y, Z -> B, 1+3+C', X, Y, Z

            # density
            density_prob = output[:, 0:1, ...]

            # image reconstruction
            if self.NeRFDecoder.img_recon_head:
                rgb_recons = output[:, 1:4, ...]
            else:
                rgb_recons = torch.zeros_like(density_prob)

            # semantic
            if self.NeRFDecoder.semantic_head:
                semantic = output[:, -self.NeRFDecoder.semantic_dim-1:-1, ...]
            else:
                semantic = torch.zeros_like(density_prob)
        ...
        if self.NeRFDecoder.semantic_head:
            ...
            loss_nerf_sem = self.NeRFDecoder.compute_semantic_loss(semantic_pred, img_semantic, lovasz=self.lovasz)
            if torch.isnan(loss_nerf):
