...
@DETECTORS.register_module()
class BEVStereo4DOCCSegmentorSparse(BEVStereo4D):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(BEVStereo4DOCCSegmentorSparse, self).__init__(**kwargs)
        self.out_dim = out_dim
        self.scene_filter_index = scene_filter_index
        out_channels = out_dim if use_predicter else num_classes
    ...
    def simple_test(self,
        ...
        img_feats, _, _ = self.extract_feat(
            points, img=img, img_metas=img_metas, **kwargs)
        occ_feature = self.final_conv(img_feats[0]).permute(0, 1, 4, 3, 2)

        # bncdhw->bnwhdc
        if self.use_predicter:
            occ_pred = self.predicter(occ_feature.permute(0, 2, 3, 4, 1))

        occ_feature = self.up_conv(occ_feature).permute(0, 2, 3, 4, 1)
    ...
    def forward_train(self,
        ...
        occ_preds = {}
        occ_pred = self.predicter(occ_feature.permute(0, 2, 3, 4, 1))
        occ_preds['original'] = occ_pred

        # upsample and calculate sparsity
        occ_pred_up = F.interpolate(occ_pred.permute(0, 4, 1, 2, 3), scale_factor=2)
        num_vx = occ_pred_up[0, 0].numel()
        sparse_shape = occ_pred_up[0, 0].shape
        occ_score = occ_pred_up.softmax(1)[:, -1].view(batch_size, -1)

        drop_rate = 1 / 0.05
        nonempty_index = occ_score.sort(-1, True)[1][:, :num_vx // drop_rate]  # [B, M]
        coords = torch.ones_like(occ_pred_up[0, 0]).nonzero().unsqueeze(0).tile((batch_size, 1, 1))   # [B, N, 4]
        occ_features_up = F.interpolate(occ_feature, scale_factor=2).permute(
            0, 2, 3, 4, 1).view(batch_size, -1, occ_feature.shape[1])    # [B, N, C]

        # generate sparse tensor batch
        coords_all = []
        occ_feature_up_all = []
        for b in range(batch_size):
            idx = nonempty_index[b]
            coord = torch.cat([torch.ones_like(idx.unsqueeze(-1)) * b, coords[b][idx]], -1)
            feature_up = occ_features_up[b]
            coords_all.append(coord)
            occ_feature_up_all.append(feature_up[idx])

        coords_all = torch.cat(coords_all)
        occ_feature_up_all = torch.cat(occ_feature_up_all)

        coords_all = coords_all.int()
        sp_tensor = SparseConvTensor(occ_feature_up_all, coords_all, sparse_shape, batch_size)

        # add sparse convlution backbone
        ## sp_tensor is a sparse tensor
        ## sp_tensor_pred = sparseconvmodule(sp_tensor).dense()
        x = self.pts_middle_encoder(occ_feature_up_all, coords_all, batch_size)
        sp_tensor_pred = sp_tensor.dense()[:, :17] # TODO replace a sparse conv module here!!
        occ_preds['upsample'] = sp_tensor_pred.permute(0, 2, 3, 4, 1)  # B, X, Y, Z, C
        ...
        # generate upsampled voxel label and mask
        occ_pred_up = self.inverse_flip_aug(occ_preds['upsample'], flip_dx, flip_dy)

        voxel_semantics = torch.zeros_like(occ_pred_up.argmax(-1)).detach()
        mask_camera = torch.zeros_like(occ_pred_up.argmax(-1)).detach()
        voxel_semantics[unq[:, 0], unq[:, 1], unq[:, 2], unq[:, 3]] = voxel_lidarseg_label.long()
        mask_camera[unq[:, 0], unq[:, 1], unq[:, 2], unq[:, 3]] = 1
        mask_camera[voxel_semantics == 0] = 0

        # occupancy losses up scale
        loss_occ_up = self.loss_single(voxel_semantics, mask_camera, occ_pred_up, semi_mask)
        losses['loss_occ_up'] = loss_occ_up['loss_occ']

        # generate original size voxel label and mask
        occ_pred = self.inverse_flip_aug(occ_preds['original'], flip_dx, flip_dy)
        voxel_semantics = F.max_pool3d(voxel_semantics.float().unsqueeze(1), 2)[:, 0].long()
        mask_camera = F.max_pool3d(mask_camera.float().unsqueeze(1), 2)[:, 0].long()

        # occupancy losses original scale
        loss_occ = self.loss_single(voxel_semantics, mask_camera, occ_pred, semi_mask)
        losses['loss_occ'] = loss_occ['loss_occ']
